...
The **Client** spec describes the client application in the terminal game: its role, connection to the server, state handling, input flow, and rendering in networked mode. It ties together [User Inputs](user-inputs/SPEC_User_Inputs.md), [Renderer](renderer/SPEC_Renderer.md), [Canvas](canvas/SPEC_Canvas.md), [Game Board](game-board/SPEC_Game_Board.md), and [Status Bar](status-bar/SPEC_Status_Bar.md) from the perspective of the running client. Details for technologies, patterns, and architecture are in this spec.

---

## 1. Client concepts and workflows

- **Mode selection:** The client can run in networked mode (connect to server) or other modes; mode determines whether state comes from the server or locally.
- **Connection lifecycle:** WebSocket to the game server; CONNECT message yields `playerId` and initial `gameState`; STATE_UPDATE messages deliver authoritative board and players; MOVE messages send local movement intent. Connection loss or server restart resets client state.
- **State ownership:** Server is authoritative. The client holds current state (board, players, entities) from the server; optionally it maintains local prediction (e.g. client-side prediction) for immediate feedback, reconciled periodically with server state.
- **Input flow:** User Inputs deliver key events; the application maps them to actions (e.g. move, quit). In networked mode, movement typically sends MOVE to the server and may update local prediction; quit cleans up and exits.
- **Rendering:** Renderer draws the Canvas (title, board, status bar). In networked mode, the client supplies current state (and, if used, predicted local position) so the Renderer can draw the correct cells and player positions. Full render on first paint or resize; incremental updates when only a few cells change (e.g. player moved).

See this spec for full patterns (message handling, state reconciliation, incremental rendering, input handling).
...
- **Incremental Rendering** ([Renderer](../renderer/SPEC_Renderer.md)): Works seamlessly with prediction
...
- **Client spec (this document)**: `docs/development/specs/terminal-game/client/SPEC_Client.md`
  - Core Concepts: "Client-Side Prediction"
  - State Management Patterns: "State Reconciliation"
